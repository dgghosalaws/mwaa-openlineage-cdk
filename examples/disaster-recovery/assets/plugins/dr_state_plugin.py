...
- Configuration is set ONCE via airflow_configuration_options
- Plugin reads DynamoDB dynamically at runtime
- Failover = update DynamoDB (takes seconds, not 20-30 minutes)

Configuration Parameters (via MWAA airflow_configuration_options):

Required:
- dr.enabled: Set to 'true' to enable DR checks (REQUIRED)
  Default: 'false'
  
Optional:
- dr.state_table: DynamoDB table name
  Default: 'mwaa-openlineage-dr-state-dev'
  
- dr.state_source: State source type - 'dynamodb' or 'parameter_store'
  Default: 'dynamodb'
  
- dr.state_table_region: AWS region where DynamoDB table is located
  Default: current region (where MWAA is running)
  IMPORTANT: For cross-region DR, set this to the primary region in BOTH regions
  Example: Set to 'us-east-2' in both us-east-2 and us-east-1 environments

Example MWAA Configuration (Primary Region - us-east-2):
{
  "dr.enabled": "true",
...
Example MWAA Configuration (Secondary Region - us-east-1):
{
  "dr.enabled": "true",
  "dr.state_table": "mwaa-openlineage-dr-state-dev",
  "dr.state_source": "dynamodb",
  "dr.state_table_region": "us-east-2"  # Same as primary - reads from primary's table
}

Usage:
1. Add this file to your MWAA plugins/ folder
2. Configure MWAA with dr.enabled=true and other parameters
...
class DRStateListener:
    ...
    def _init_dynamodb(self):
        ...
        # Get table region (defaults to current region if not specified)
        try:
            self.table_region = conf.get('dr', 'state_table_region', fallback=self.current_region)
        except:
            self.table_region = os.environ.get('AIRFLOW__DR__STATE_TABLE_REGION', self.current_region)
        ...
        try:
            ...
            logger.info(
                ...
                f"current_region: {self.current_region}"
            )
        except Exception as e:
